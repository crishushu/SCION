...
function transformStateNode(node, ancestors) {
    ...
    id = pm.platform.dom.hasAttribute(node,"id") 
    ? pm.platform.dom.getAttribute(node,"id") 
    :  genId(pm.platform.dom.localName(node)); 

    if (pm.platform.dom.hasAttribute(node,"id") && kind !== stateKinds.HISTORY) {
      state = {
          id: id,
          kind: kind,
          descendants: [],
          name: pm.platform.dom.getAttribute(node, "name"),
          type: pm.platform.dom.getAttribute(node, "type")
      };
    } else {
      state = {
        id: id,
